...
int main(int argc, char **argv)
{
	...
	}else
	{
		if( my_rank == 0 )
		{
			...
			for( size_t target = 1; target < (size_t)process_count; ++target )	
			{
				MPI_Send(&start, 1, MPI_UNSIGNED_LONG_LONG, target /*destino*/, 0 /*tag*/, MPI_COMM_WORLD);	
				MPI_Send(&finish, 1, MPI_UNSIGNED_LONG_LONG, target /*destino*/, 0 /*tag*/, MPI_COMM_WORLD);	
				// MPI_Send(&record, sizeof(record), MPI_BYTE, target /*destino*/, 0 /*tag*/, MPI_COMM_WORLD);	
			}
		}else
		{
				MPI_Recv(&start, 1, MPI_UNSIGNED_LONG_LONG, 0 /*source*/, 0 /*tag*/, MPI_COMM_WORLD, MPI_STATUS_IGNORE);	
				MPI_Recv(&finish, 1, MPI_UNSIGNED_LONG_LONG, 0 /*source*/, 0 /*tag*/, MPI_COMM_WORLD, MPI_STATUS_IGNORE);	
		}
	}
